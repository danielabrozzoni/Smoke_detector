...
HANDLE COM_open(int port, char mode, int speed, char parity, int bits, int stop, char flow)
{
    ...
    fileHandle = CreateFile(
         ...
         GENERIC_READ | GENERIC_WRITE,//Apertura il lettura / scrittura
            0, //sharing mode, se 0 la porta non pu� essere condivisa
            0, //attributi di sicurezza, con 0 l'handle non pu� essere ereditato
            OPEN_EXISTING, //la porta deve esistere per poter essere aperta, altrimenti non pu� crearla
            0, //Overlapped /non-overlapped mode, con 0 non -overlapped (non si pu� leggere e scrivere contemporaneamente)
            0 //template file, non definito per le seriali
        );
    //Controllo se l'apertura � fallita
    ...
    //Costruisco una stringa che � il parametro d'ingresso per settare i campi di un DCB
    //La stringa contiene i parametri speed, parity, data size, stop bits separati da virgola
    //speed � in BAUD
    //parity � n per nessuna parit�, e(even) per pari, o(odd) per dispari
    //data size � il numero di bit (tipicamente 8)
    //stop bits � il numero di bit di stop, tipicamente 1 o 2
    ...
    //Se passa pi� di quel tempo la lettura � considerata conclusa e la
    ...
    //� impostato a 10 il timeout scatta dopo un secondo
